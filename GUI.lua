...
-- ALL settings + Anti-Admin Server Hop support
-- ============================================
local GUI = {}
local Config = nil
local Library = nil
...
function GUI.Init(cfg)
    ...
    FarmTab:AddToggle("Highest Value Mode", Config.Farm.CurrentMode == "Highest", function(state)
    ...
    FarmTab:AddSlider("Collect Distance", 1, 10, S.Farming.CollectDistance, function(val)
        S.Farming.CollectDistance = val; Config.SaveSettings()
    end)
    ...
    SafetyTab:AddSlider("Safety Radius", 5, 100, S.Farming.SafetyRadius, function(val)
        S.Farming.SafetyRadius = val; S.Farming.ResumeRadius = val + 5; Config.SaveSettings()
    end)
    ...
    SafetyTab:AddButton("Cycle Flee Mode (Armed > All > Off)", function()
        ...
        Config.SaveSettings(); Notify("Flee", "Mode: " .. S.Farming.FleeMode, 1.5)
    end)

    SafetyTab:AddSeparator()
    SafetyTab:AddSection("Anti-Admin Protection")

    SafetyTab:AddToggle("Anti-Admin", S.Farming.AntiAdmin, function(state)
        S.Farming.AntiAdmin = state; Config.SaveSettings()
        Notify("Anti-Admin", state and "ON" or "OFF", 2, state and "success" or "warning")
    end)

    -- Anti-Admin Action Mode
    local actionLabel = SafetyTab:AddLabel("Action: " .. (S.Farming.AntiAdminAction or "Kick"))
    SafetyTab:AddButton("Toggle Action (Kick / Server Hop)", function()
        if S.Farming.AntiAdminAction == "Kick" then
            S.Farming.AntiAdminAction = "ServerHop"
        else
            S.Farming.AntiAdminAction = "Kick"
        end
        actionLabel.Text = "Action: " .. S.Farming.AntiAdminAction
        Config.SaveSettings()
        Notify("Anti-Admin", "Action: " .. S.Farming.AntiAdminAction, 2)
    end)

    -- Auto Re-Execute after server hop
    SafetyTab:AddToggle("Auto Re-Execute (after hop)", S.Farming.AutoReExecute, function(state)
        S.Farming.AutoReExecute = state; Config.SaveSettings()
        Notify("Re-Execute", state and "Script will re-run after hop" or "Disabled", 2, state and "success" or "warning")
    end)

    -- Auto Start Farm after re-execute
    SafetyTab:AddLabel("(Auto Start Farm is in Extra tab)")

    SafetyTab:AddSeparator()
    SafetyTab:AddSection("Other Protection")

    SafetyTab:AddToggle("TP Protection (Flee)", S.Movement.TPProtection, function(state)
        S.Movement.TPProtection = state; Config.SaveSettings()
    end)

    SafetyTab:AddToggle("Anti-AFK", S.Movement.AntiAFK, function(state)
        S.Movement.AntiAFK = state; Config.SaveSettings()
    end)

    -- ══════════════════════════════════════
    -- MOVEMENT TAB
    -- ══════════════════════════════════════
    local MoveTab = Library:AddTab("Move", "")

    MoveTab:AddSection("Speed")
    MoveTab:AddSlider("Walk Speed", 10, 100, S.Movement.WalkSpeed, function(val)
        S.Movement.WalkSpeed = val; Config.SaveSettings()
    end)
    MoveTab:AddSlider("Stuck Threshold", 1, 10, S.Movement.StuckThreshold, function(val)
        S.Movement.StuckThreshold = val; Config.SaveSettings()
    end)

    MoveTab:AddSeparator()
    MoveTab:AddSection("Teleport")
    MoveTab:AddToggle("Cheat TP (near target)", S.Movement.CheatTP, function(state)
        S.Movement.CheatTP = state; Config.SaveSettings()
    end)
    ...
    local colorPicker = VisTab:AddColorPicker("Static Color", S.Visuals.PathColor, function(c)
        S.Visuals.PathColor = c; S.Visuals.ColorMode = "Static"; Config.SaveSettings()
    end)
    ...
    VisTab:AddButton("Play Custom Cycle (use slots below)", function()
        if #S.Visuals.CustomColors < 1 then Notify("Color", "Add colors to slots first!", 2, "error"); return end
    ...
    VisTab:AddSlider("Color Speed", 1, 10, math.floor(S.Visuals.ColorSpeed), function(val)
        S.Visuals.ColorSpeed = val; Config.SaveSettings()
    end)
    VisTab:AddToggle("Smooth Fade", S.Visuals.ColorFade, function(state)
        S.Visuals.ColorFade = state; Config.SaveSettings()
    end)

    VisTab:AddSeparator()
    VisTab:AddSection("Custom Color Cycle Slots")
    local colorSlots = VisTab:AddColorSlots(10, S.Visuals.CustomColors,
        function() return S.Visuals.PathColor end,
        function(idx, color, allColors) S.Visuals.CustomColors = allColors; Config.SaveSettings(); Notify("Slot "..idx, "Color saved", 1, "success") end,
        function() S.Visuals.CustomColors = {}; Config.SaveSettings(); Notify("Slots", "All cleared", 1.5, "warning") end
    )
    ...
    WebTab:AddTextbox("URL", S.Webhook.URL ~= "" and S.Webhook.URL or "", function(text)
        S.Webhook.URL = text; Config.SaveSettings()
    end)
    WebTab:AddToggle("Enable Webhook", S.Webhook.Enabled, function(state)
        S.Webhook.Enabled = state; Config.SaveSettings()
    ...
    WebTab:AddSlider("Interval", 1, 60, S.Webhook.Interval, function(val)
        S.Webhook.Interval = val; Config.SaveSettings()
    end)
    ...
    WebTab:AddButton("Cycle Unit (Sec > Min > Hr)", function()
        ...
        unitLabel.Text = "Unit: " .. S.Webhook.Unit; Config.SaveSettings()
    end)

    WebTab:AddSeparator()
    WebTab:AddButton("Send Test Webhook", function()
        if Config.Modules.Webhook then Config.Modules.Webhook.Send(false); Notify("Webhook", "Test sent!", 2, "success") end
    end)
    ...
    ZoneTab:AddAccentButton("Set Pos 1 (current position)", function()
        Config.Zone.Pos1 = Config.RootPart.Position; p1Label.Text = "Pos 1: SET"
    ...
    ZoneTab:AddAccentButton("Set Pos 2 (current position)", function()
        Config.Zone.Pos2 = Config.RootPart.Position; p2Label.Text = "Pos 2: SET"
    ...
    ExtraTab:AddToggle("Loop Buy Case 26", S.Extra.LoopBuyCase26, function(state)
        S.Extra.LoopBuyCase26 = state; Config.SaveSettings()
        Notify("Case 26", state and "Auto-buying ON" or "Auto-buying OFF", 2, state and "success" or "warning")
    end)

    ExtraTab:AddToggle("Auto Start Farm (next join/hop)", false, function(state)
        S.Farming.AutoStartFarm = state; Config.SaveSettings()
    ...
    ExtraTab:AddButton("Reset UI Position", function()
        Library.MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0); Notify("UI", "Position reset", 1.5)
    end)
    ...
    task.wait(0.5)
    Notify("Heart Farm V1", "Loaded! Press RightShift to toggle UI.", 4, "success")
end
